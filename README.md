...
- Параметр `--nostd`, который всё лишнее из стандартной библиотеки, такое как ввод-вывод. Также уменьшает размер кода и делает некоторые оптимизации для запуска на микроконтроллерах.
# Не забыть
in, is как в python
