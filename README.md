...
- Компилятор может сжать файлы и помещать внутрь exe (как в apk на android) с помощью ключевого слова `include`. По сути в exe поместится байтовый массив. Изменять их можно, но они не сохранятся, т.к. они зашиты в exe.
### Целевые применения
- Написание серверов
...
- Программа запускается из главного файла. Если нужно принять параметры на вход программы, пишется функция `main(str[] args)`. Запускаемый файл по стандарту должен называться **`main.aq`** или **`main.aqua`**. Скомпилированный файл будет называться **`main.water`**. Он содержит в себе синтаксическое дерево в сжатом виде. Такой файл можно открыть с помощью программы, поставляемов в пакете **aqua**. Такие файлы может отобразиться в двух представлениях: стандартный валидный aqua код или aqua код с дополнительной информацией, такой как рассчитанные типы переменных. Скомпилированная статическая библиотека - **`library.snow`**, динамическая библиотека - **`library.ice`**
...
- `/regex/` - Regex
- `"string"` - String
- `'c'` - char. Если в одинарных ковычках `'` будет больше 1 символа, они воспримутся как строка.
## Примеры кода
### Hello world
```dart
println("Hello, world!")
