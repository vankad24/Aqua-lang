...
- Существует 3 вида сущностей: **object, read-only reference, mutable reference**.
```dart
a = Animal() //object
...
- Цикл for - `for (var_name : Number) { ... }` Пример: `for (x : arr) { ... }`, где для `x` автоматически определится тип данных исходя из массива `arr`. Можно менять начальное и конечное значение: `for (i = 3 : arr.len-1) { ... }` равносильно `for(i = 3; i<arr.len-1; i++){ ... }`. Если индекс `i` не нужен, можно написать `for(5){ ... }`. Для итерации в обратном порядке или с другим шагом необходимо использовать обычный цикл for
- Цикл foreach может применяться к объектам, которые реализовали интерфейс `Iterable`. Примеры: `for (x : arr)`, `for (i, value: arr)` - как enumerate в python, `for (key, value : dict.pairs())` - с распаковкой
