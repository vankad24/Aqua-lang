...
- `Nullable` - объекты этого класса создаются при использовании `null`.
  ...
  - `value()` - возвращает объекта, если он есть. Иначе вызывает `NullValueException`. Оператор `?` просто вызывает эту функцию.
...
n_user = Nullable<User>() //не содержит объект, это null
```
  - Краткая запись:
```dart
User? n_user
