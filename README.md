...
- `$` - оператор копирования (deep copy). Работает как конструктор: инициализирует все поля, если не указано. Также может продлить жизнь объекту (он не будет уничтожен при следующем выходе из области видимости).
- `!=` - логическое "не равно"
