...
- Цикл foreach - `for (var_name : iterable/Number) { ... }` Пример: `for (x : arr) { ... }`, где для `x` автоматически определится тип данных исходя из массива `arr`. Можно менять начальное и конечное значение: `for (i = 3 : arr.len-1) { ... }` равносильно `for(i = 3; i<arr.len-1; i++){ ... }`. Для итерации в обратном порядке или с другим шагом необходимо использовать обычный цикл for. Пример с распоковкой: `for (key, value : dict.pairs())`.
...
- `Array` - стандартный неизменяемый массив. Реализует iterable. Конструктор может принимать iterable.
  ...
  - Конструктор `Array.raw()` не инициализирует значения, а оставляет те, которые были в памяти.
...
- `Operators` - класс, содержащий функции-операторы
  - `Operators._+("hi",5)` равносильно `"hi"+5`
## ООП
