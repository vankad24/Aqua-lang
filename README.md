...
- `File` - класс для чтения и записи в файл (как `open` в python)
  ...
  - Статический метод `write(str filename, str data, FileMode mode = R)`, который просто сохраняет строку `data` в файл и закрывает его.
  - Статический метод `read(str filename, FileMode mode = W)`, который возвращает текст из файла в виде `str`.
