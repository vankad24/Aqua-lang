...
- При объявлении переменной можно указывать конкретно val или var (let), чтобы сделать её константой или изменяемой. Если не указывать конкретно, например `a = 24`, то при первом появлении переменной `a` это равносильно `var a = 24`. Чтобы не допустить повторное определение, лучше писать `var`, поскольку, если 2 раза написать `var a = 24` компилятор выдаст ошибку. Чтобы запретить короткую инициализацию, такую как `a = 24` и принудить писать `var`, компилятору нужно передать флаг `allowShortVarInit=false`.
- Переменные можно объявлять несколькими способами. Объявление без инициализации: `Float num` и `val Float num` для константы. Объявление с инициализацией: `num = 2.4f` - короткая запись, `Float num = 2.4` - автоматическое приведение, `num = Float(2.4)` - с помощью конструктора.
### Операторы
...
- `File` - класс для чтения и записи в файл (как `open` в python)
  ...
  - Статический метод `write(str filename, str data, FileMode mode = W)`, который просто сохраняет строку `data` в файл и закрывает его.
...
- Флаг компилятора strictMode=true, который ставит allowShortVarInit=false и подобные переменные.
# Не забыть
in, is как в python
...
Ключевое слово init, как в kotlin?
...
Вместо указателей Memory класс
