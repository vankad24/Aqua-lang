...
- Для циклов `for` с целыми числами можно написать `pauseon`, для исполнения кода на какой-то итерации.
```dart
for i : 10 {
	println(i)
}pauseon(5){
...
- Обработка ошибок здесь работает другим образом. Нет ключевого слово `try`, есть только `catch`. При вызове исключения выполнение будет не всплывать, как это в других языках, а будет идти по коду ниже до ближайшего блока `catch`. Если в текущей функции этого блока нет, произойдёт всплытие.
```
file.open("some_path")
...
- `Any` - класс, от которого неявно все наследуются (как Object в java). Имеет стандартно реализованные функции:
  ...
  - `bytes` - массив байтов, который объект занимает в памяти
...
```
Конструкция `static once` означает, что фрагмент кода запустится один раз за всю программу. Просто `once` означает, что фрагмент кода запустится один раз в этой функции.
В данном случае нам необходимо проверить условие `if` только один раз, а потом его просто пропускать (делать jump в коде). Это можно реализовать следующим образом: адрес, куда будет выполнен прыжок хранится в переменной и изначально указывает на следующую строчку. В конце нашего блока кода `once` исполнится код, который поменяет адрес прыжка, и в следующий раз весь этот кусок кода будет пропущен.
## Литералы
### Визуальные
...
В логотипе сделать букву А, как логотип в стартреке. Каплю с водой оставить
...
Позволить возвращать null, но обязывать проверять его. Как java обязывает обрабатывать исключения