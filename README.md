...
- Специальный `if`, под названием `ifonce`. Он проверяется лишь один раз, а дальше весь этот кусок кода пропускается. Это полезно, например, при реализации singletone.
```dart
getInstance(){
	ifonce(!instance.exists())
		instance.create()
	return instance
}
```
