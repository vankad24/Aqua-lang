...
- Инициализация переменной: `a = Animal(24)`, объявление переменной - `Animal a`
...
- `?` - проверяет на null. `if(?value)` равносильно `if(value != null)`. Не может быть переопределён.
...
- `!!` - оператор копирования. Код `arr1 = arr2` запишет в `arr1` ссылку на `arr2`, а код `arr1 = !!arr2` скопирует объект `arr2` и запишет ссылку на копию в `arr1`
...
- `Json` - тот самый json (наследуется от Dict<str,Json>)
  ...
  - `array()` - возвращает `Array<Json>.map(=>arg.str())`.
...
- `String` - является массивом из `char`
  ...
  - Поддержка отрицательных индексов
  - Нет разницы между `'` и `"`. Если нужно создать символ, можно использовать литерал или явный тип: `char c = "!"`
...
- `arg` - ссылка на первый аргумент функции. Имеет конкретный тип
- `alias` - переименование функций и пакетов. Действует только в текущем файле. Пример: `alias текущее_имя новое_имя`. По умолчанию неявно применён `alias Integer int`, `alias Float float`, `alias String str`, `alias UnsignedInt uint` и т.д.
