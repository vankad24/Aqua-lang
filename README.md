...
```
В данном случае нам необходимо проверить условие `if` только один раз, а потом его просто пропускать (делать jump в коде). Это можно реализовать следующим образом: место, куда будет выполняться в jump хранится в переменной и изначально указывает на следующую строчку. В конце нашего блока кода `once` выполняется код, который меняет место прыжка, пропуская весь этот кусок кода.
## Литералы
### Визуальные
