...
- Компилятор бы мог не только собирать исполняемый файл, но и выдавать промежуточный код, в котором можно просмотреть, какие инструкции вызываются. Такие файлы имели бы расширение **.H2O** - аналогия с мельчайшей частичкой воды.
### Целевые применения
- Написание серверов
...
a3 = a2  //read-only reference
a4 = &a  //mutable reference
a5 = a4  //read-only reference
a6 = &a4 //mutable reference, but error, can be only one mutable reference
a7 = $a  //copied object
a2 = a4  //create read-only reference from a4 and assign to a2
a4 = a2  //error

Animal ref1 = a  //read-only reference
Animal ref2 = &a //mutable reference

a_obj = Animal() //object
a_obj = a    //copy to a_obj
a_obj = ref1 //copy to a_obj
a_obj = ref2 //copy to a_obj
a_obj = &a   //error
a2 = a_obj   //rewrite read-only reference
a2 = $ref1   //redefine object a
...
bar1(ref1, ref1) //correct
bar1(ref2, a)    //correct
bar1(a, a)       //correct
bar2(ref2, a)    //error, 2 mutable references
bar2(a, a)       //error, 2 mutable references
bar2(a, a_obj)   //correct
bar2(a, $a)      //correct
```
...
### Общее
- Программа запускается из главного файла. Если нужно принять параметры на вход программы, пишется функция `main(str[] args)`. Запускаемый файл по стандарту должен называться **`main.aq`**. Скомпилированный object файл будет называться **`main.water`**. Скомпилированная статическая библиотека - **`library.snow`**, динамическая библиотека - **`library.ice`**
...
- `String` - является массивом из `char`. Является Iterable
  ...
  - Нет разницы между `'` и `"`. Чтобы создать символ, нужно использовать явный тип `char c = "!"` или cast `c = "!":char`
...
- `Pointer` - указатель. Абстрактный класс.
  ...
  - Внутри классов можно вызвать `alloc()` для получения указателя. Этот метод неявно вызовется у стандартного аллокатора или у иного, если он был указан при инициализации.
...
- `ImmutRef` - класс неизменяемых ссылок. Все неизменяемые ссылки являются объектами этого класса.
  - Стандартный синтаксис `ref = ImmutRef<Animal>(a)`
  - Короткий синтаксис `ref = a`
  - Может использоваться для создания массива ссылок: `Array<ImmutRef<Any>> arr`. Функция println принимает именно такой параметр.
- `Ref` - класс изменяемых ссылок. Все ссылки являются объектами этого класса.
...
- К private и protected полям можно обращаться через `_`, но для этого нужно использовать ключевое слово `unlock`
...
- Дженерики. Пишутся перед функцией или после имени класса. Их можно сокращать: `Array<Array<int>>` равносильно `Array<<int>>`. Можно присваивать: `foo<T>(Class cls){ T = cls }`
...
- При создании локальных переменных того-же класса можно напрямую обращаться к private полям. Снаружи так можно делать только через `unlock` и `_`
...
- `import` - подключение пакетов. Поддержка `as` (как в python), а так же `safely`. По умолчанию `import` добавляет всё в текущее пространство имён. То есть после `import Math` можно вызывать `sin(.5)`, но если использовать `import safely Math` нужно писать `Math.sin(.5)`. Если нет конфликта имён, можно писать только название класса, оно само найдет до него путь. Если есть конфликт, нужно писать путь до пакета `import aqua.Math` или `import lib.Math`. Можно и импортировать весь пакет: `import aqua.utils.*`
...
- `unlock` - пишется перед вызовом конструктора. Разрешает обращаться к private полям объекта через `_`
## Литералы
...
- `0x` - hex
- `\u` - для символов: `\u00F7`
- Вместо типовых литералов используется cast или явное указание: `byte b = 10`, `c = "~":char`. Если переменная слишком большая для типа, она обрежется: `byte b = 1000` обрежется до 127.
- Для чисел тип определяется автоматически, исходя из значения. Сначала выбирается int, затем, если не хватает, берётся long, затем blong, затем BigNumber.
